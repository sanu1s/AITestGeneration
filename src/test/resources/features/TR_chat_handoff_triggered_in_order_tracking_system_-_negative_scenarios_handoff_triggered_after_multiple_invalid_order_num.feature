Feature: Chat Handoff Triggered in Order Tracking System - Negative Scenarios
  ...
  Scenario: Handoff triggered after multiple invalid order number attempts
    Given the user is on the order tracking page
    When the user enters order number "INVALID_A1"
    And the user clicks the "Track Order" button
    And the user enters order number "INVALID_A2"
    And the user clicks the "Track Order" button
    And the user enters order number "INVALID_A3"
    And the user clicks the "Track Order" button
