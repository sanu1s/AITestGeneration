...
public class TranscriptPipelineUsingGherkins {

    // Define AI Service for Use Case generation
    interface UseCaseAssistant {
        @SystemMessage("""
        You are a Senior QA Engineer. Extract use cases from transcripts to generate the test cases.
        For the 'gherkinSteps' field, strictly use BDD format:
        - Start steps with Given, When, or Then.
        - Ensure they describe a logical test scenario.
        """)
        UseCaseReport analyze(String transcript);
    }
    ...
    public static void main(String[] args) {
        ...
        for (Row row : rows) {
            ...
            if (cleanText != null && !cleanText.trim().isEmpty()) {
                ...
                UseCaseReport result = assistant.analyze("Identify use cases from this transcript: " + cleanText);
                System.out.println("AI Analysis:\n" + result);
                // 2. Access specific fields directly
                System.out.println("USE CASE: " + result.title);
                System.out.println("ACTORS: " + String.join(", ", result.actors));

                // 3. Print the Gherkin steps for your test cases
                System.out.println("GHERKIN STEPS:");
                result.gherkinSteps.forEach(step -> System.out.println("  " + step));
            }
        }
